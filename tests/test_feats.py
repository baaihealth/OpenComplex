...
class TestFeats(unittest.TestCase):
    ...
    def test_pseudo_beta_fn_compare(self):
        def test_pbf(butype, all_atom_pos, all_atom_mask):
        ...
        butype = np.random.randint(0, 22, (n_res,))
        ...
        out_gt_pos, out_gt_mask = f.apply(
            {}, None, butype, all_atom_pos, all_atom_mask
        )
    ...
    def test_atom37_to_torsion_angles_compare(self):
        def run_test(butype, all_atom_pos, all_atom_mask):
        ...
        butype = np.random.randint(0, 22, (n_templ, n_res)).astype(np.int64)
    ...
    def test_atom37_to_frames_compare(self):
        def run_atom37_to_frames(butype, all_atom_positions, all_atom_mask):
            return alphafold.model.all_atom.atom37_to_frames(
                butype, all_atom_positions, all_atom_mask
            )
        ...
        batch = {
            "butype": np.random.randint(0, 21, (n_res,)),
    ...
    def test_torsion_angles_to_frames_compare(self):
        def run_torsion_angles_to_frames(
            butype, backb_to_global, torsion_angles_sin_cos
        ):
        ...
        butype = np.random.randint(0, 21, size=(n_res,))
        ...
        out_gt = f.apply({}, None, butype, rigids, torsion_angles_sin_cos)
    ...
    def test_frames_and_literature_positions_to_atom14_pos_compare(self):
        ...
        out_gt = f.apply({}, None, butype, rigids)
        jax.tree_map(lambda x: x.block_until_ready(), out_gt)
        out_gt = torch.stack(
