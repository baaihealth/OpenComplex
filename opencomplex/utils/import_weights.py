...
class ParamType(Enum):
    LinearWeight = partial(  # hack: partial prevents fns from becoming methods
        lambda w: w.transpose(-1, -2) if w.ndim > 1 else w.unsqueeze(-1)
    )
    ...
    LinearWeightMultimer = partial(
        lambda w: rearrange(w, 'a ... -> (...) a')
    )
    LinearBiasMultimer = partial(
        lambda w: torch.flatten(w)
    )
...
def generate_translation_dict(model, version):
    ...
    LinearParamsMultimerNoBias = lambda l: {
        "weights": LinearWeightMultimer(l.weight),
    }
    ...
    if not is_multimer:
        ...
        IPAParams = lambda ipa: {
            "q_scalar_projection": LinearParamsMultimerNoBias(ipa.linear_q),
            "k_scalar_projection": LinearParamsMultimerNoBias(ipa.linear_k),
            "v_scalar_projection": LinearParamsMultimerNoBias(ipa.linear_v),
            "q_point_projection": {'point_projection': LinearParamsMultimer(ipa.linear_q_points)},
            "k_point_projection": {'point_projection': LinearParamsMultimer(ipa.linear_k_points)},
            "v_point_projection": {'point_projection': LinearParamsMultimer(ipa.linear_v_points)},
            "trainable_point_weights": Param(
                param=ipa.head_weights, param_type=ParamType.Other
            ),
            "attention_2d": LinearParams(ipa.linear_b),
    ...
    def FoldIterationParams(sm):
        ...
        if is_multimer:
            ret.pop("affine_update")
            ret["quat_rigid"] = {"rigid": LinearParams(sm.bb_update.linear)}
    ...
    translations = {
        "evoformer": {
            ...
            "pair_activiations": LinearParams(
                model.input_embedder.rpe.linear_relpos
            ),
    ...
    if is_multimer:
        translations["evoformer"].pop("pair_activiations")
        ...
        if not is_multimer:
            ...
            template_embedding = {
                "single_template_embedding": {
                    ...
                    "template_pair_embedding_0": LinearParams(model.template_pair_embedder.dgram_linear),
                    "template_pair_embedding_1": LinearParams(model.template_pair_embedder.pseudo_beta_mask_linear),
                    "template_pair_embedding_2": LinearParams(model.template_pair_embedder.butype_linear_1),
                    "template_pair_embedding_3": LinearParams(model.template_pair_embedder.aatype_linear_2),
                    "template_pair_embedding_4": LinearParams(model.template_pair_embedder.x_linear),
                    "template_pair_embedding_5": LinearParams(model.template_pair_embedder.y_linear),
                    "template_pair_embedding_6": LinearParams(model.template_pair_embedder.z_linear),
                    "template_pair_embedding_7": LinearParams(model.template_pair_embedder.backbone_mask_linear),
                    "template_pair_embedding_8": LinearParams(model.template_pair_embedder.query_embedding_linear),
                    "query_embedding_norm": LayerNormParams(model.template_pair_embedder.query_embedding_norm),
                },
                "output_linear": LinearParams(model.template_pointwise_att_replaced_linear),
            }
        template_param_dict = {
    ...
    if "_ptm" in version or model.config["heads"].tm.enabled:
