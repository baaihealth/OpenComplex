...
from opencomplex.model.primitives import Linear, LayerNorm, Attention
from opencomplex.model.dropout import (
    DropoutRowwise,
    DropoutColumnwise,
)
...
from opencomplex.model.triangular_multiplicative_update import (
    TriangleMultiplicationOutgoing,
    TriangleMultiplicationIncoming,
)
from opencomplex.utils.checkpointing import checkpoint_blocks
from opencomplex.utils.chunk_utils import (
    chunk_layer,
    ChunkSizeTuner,
)
...
def embed_templates_offload(
    ...
    pair_embeds_cpu = []
    n = z.shape[-2]
    n_templ = batch["template_butype"].shape[templ_dim]
    for i in range(n_templ):
        idx = batch["template_butype"].new_tensor(i)
    ...
    if(inplace_safe):
        ...
        t = torch.einsum('i...,i->i...', t, torch.sum(batch["template_mask"], dim=-1) > 0)
...
def embed_templates_average(
    ...
    if(inplace_safe):
        ...
        out_tensor = torch.einsum('i...,i->i...', out_tensor, torch.sum(batch["template_mask"], dim=-1) > 0)
...
def embed_templates(model, batch, z, pair_mask, templ_dim, inplace_safe, multichain_mask): 
    if(model.template_config.offload_templates):
        return embed_templates_offload(model, 
            batch, z, pair_mask, templ_dim, inplace_safe=inplace_safe,
        )
    elif(model.template_config.average_templates):
        return embed_templates_average(model, 
            batch, z, pair_mask, templ_dim, inplace_safe=inplace_safe,
        )
