...
def get_atom_coords(
    ...
    all_atom_positions = np.zeros(
        [num_res, constants.atom_type_num, 3], dtype=np.float32
    )
    all_atom_mask = np.zeros(
        [num_res, constants.atom_type_num], dtype=np.float32
    )
    for residue_index in range(num_res):
        pos = np.zeros([constants.atom_type_num, 3], dtype=np.float32)
        ...
        if not res_at_position.is_missing:
            ...
            for atom in res.get_atoms():
                ...
                if atom_name in constants.atom_order.keys():
                    pos[constants.atom_order[atom_name]] = [x, y, z]
                    mask[constants.atom_order[atom_name]] = 1.0
                elif atom_name.upper() == "SE" and res.get_resname() == "MSE":
                    # Not sure whether there will be selenium atom in RNA structure
                    # Put the coords of the selenium atom in the sulphur column
                    pos[residue_constants.atom_order["SD"]] = [x, y, z]
                    mask[residue_constants.atom_order["SD"]] = 1.0

        all_atom_positions[residue_index] = pos
        all_atom_mask[residue_index] = mask
