...
from opencomplex.data import templates, parsers, mmcif_parsing
...
def make_template_features(
    ...
    if(len(hits_cat) == 0 or template_featurizer is None):
        ...
        if(template_features["template_butype"].shape[0] == 0):
            template_features = empty_template_feats(len(input_sequence))
...
def unify_template_features(
    ...
    out_dicts = []
    seq_lens = [fd["template_butype"].shape[1] for fd in template_feature_list]
    for i, fd in enumerate(template_feature_list):
        out_dict = {}
        n_templates, n_res = fd["template_butype"].shape[:2]
        for k,v in fd.items():
            seq_keys = [
                "template_butype",
                "template_all_atom_positions",
...
def make_mmcif_features(
    ...
    all_atom_positions, all_atom_mask = mmcif_parsing.get_atom_coords(
        mmcif_object=mmcif_object, chain_id=chain_id, chain_type=chain_type
    )
...
def make_msa_features(
    ...
    features["num_alignments"] = np.array(
        [num_alignments] * num_bu, dtype=np.int32
    )
    return features
...
class DataPipeline:
    ...
    def process_prepared_features(
        ...
        for k in features:
            if features[k].ndim == 0:
                features[k] = np.repeat(features[k], seq_len)

        # NOTE: wrong name for precompute features
        if "template_all_atom_masks" in features:
            features["template_all_atom_mask"] = features.pop("template_all_atom_masks")

        return features

    def process_mmcif_with_prepared_feature(
        self,
        ...
        if chain_id is None:
            if not "asym_id" in features:
                ...
                if features['butype'].ndim > 1:
                    features['butype'] = np.argmax(features['butype'], axis=-1)
                chain_id = []
                chains = copy.deepcopy(mmcif.chain_to_seqres)
        ...
        binary_mask = mmcif_feats['all_atom_mask'].astype(bool)
        translation_vec = mmcif_feats['all_atom_positions'][binary_mask].mean(axis=0)
