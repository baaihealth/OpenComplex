...
from opencomplex.data.data_pipeline import DataPipeline
from opencomplex.data.templates import TemplateHitFeaturizer
...
class TestDataPipeline(unittest.TestCase):
    ...
    def test_fasta_compare(self): 
        ...
        opencomplex_feature_dict = data_pipeline.process_fasta(
            "tests/test_data/short.fasta", 
            "tests/test_data/alignments"
        )

        opencomplex_feature_dict["template_all_atom_masks"] = opencomplex_feature_dict["template_all_atom_mask"]

        checked = []

        # AlphaFold and opencomplex process their MSAs in slightly different
        # orders, which we compensate for below.
        m_a = alphafold_feature_dict["msa"]
        m_o = opencomplex_feature_dict["msa"]
        ...
        rearranged_o_dmi = opencomplex_feature_dict["deletion_matrix_int"]
        ...
        for k, v in alphafold_feature_dict.items():
            self.assertTrue(
                k in checked or np.all(v == opencomplex_feature_dict[k])
            )
