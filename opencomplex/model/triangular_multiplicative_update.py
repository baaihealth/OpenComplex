...
from opencomplex.utils.tensor_utils import add, permute_final_dims
from opencomplex.faster_alphafold.faster_alphafold_config import faster_alphafold_config
from opencomplex.faster_alphafold.faster_alphafold import TriangleUpdateABG
...
class TriangleMultiplicativeUpdate(nn.Module):
    ...
    def __init__(self, c_z, c_hidden, _outgoing=True):
        ...
        if faster_alphafold_config.triangle_multiplicative_update:
            linear_a_p = Linear(self.c_z, self.c_hidden)
            linear_a_g = Linear(self.c_z, self.c_hidden, init="gating")
            linear_b_p = Linear(self.c_z, self.c_hidden)
            linear_b_g = Linear(self.c_z, self.c_hidden, init="gating")
            linear_g = Linear(self.c_z, self.c_z, init="gating")
            self.linear_a_pg_b_pg_g = Linear(self.c_z, self.c_hidden * 4 + self.c_z)
            self.linear_a_pg_b_pg_g.weight.data.copy_(torch.cat((linear_a_p.weight.data, linear_a_g.weight.data, linear_b_p.weight.data, linear_b_g.weight.data,
                linear_g.weight.data), 0))
            self.triangle_update_abg = TriangleUpdateABG(self.c_z, self.c_hidden)
        else:
    ...
    def _init_inference_weights(self):
        # unpack merged weights for inference
        self.linear_a_p = Linear(self.c_z, self.c_hidden)
        self.linear_a_g = Linear(self.c_z, self.c_hidden, init="gating")
        self.linear_b_p = Linear(self.c_z, self.c_hidden)
        self.linear_b_g = Linear(self.c_z, self.c_hidden, init="gating")
        ...
        weight = self.linear_a_pg_b_pg_g.weight
        bias = self.linear_a_pg_b_pg_g.bias
        self.linear_a_p.weight.data.copy_(weight.data[:self.c_hidden])
        self.linear_a_g.weight.data.copy_(weight.data[self.c_hidden:self.c_hidden*2])
        self.linear_b_p.weight.data.copy_(weight.data[self.c_hidden*2:self.c_hidden*3])
        self.linear_b_g.weight.data.copy_(weight.data[self.c_hidden*3:self.c_hidden*4])
        self.linear_g.weight.data.copy_(weight.data[self.c_hidden*4:])
        self.linear_a_p.bias.data.copy_(bias.data[:self.c_hidden])
        self.linear_a_g.bias.data.copy_(bias.data[self.c_hidden:self.c_hidden*2])
        self.linear_b_p.bias.data.copy_(bias.data[self.c_hidden*2:self.c_hidden*3])
        self.linear_b_g.bias.data.copy_(bias.data[self.c_hidden*3:self.c_hidden*4])
        self.linear_g.bias.data.copy_(bias.data[self.c_hidden*4:])

        self.linear_a_p.to(weight.device)
        self.linear_a_g.to(weight.device)
        self.linear_b_p.to(weight.device)
        self.linear_b_g.to(weight.device)
        self.linear_g.to(weight.device)

        self.linear_a_p.type(weight.dtype)
        self.linear_a_g.type(weight.dtype)
        self.linear_b_p.type(weight.dtype)
        self.linear_b_g.type(weight.dtype)
        self.linear_g.type(weight.dtype)

    def _purge_inference_weights(self):
        if not hasattr(self, 'linear_a_pg_b_pg_g'):
            return
        del self.linear_a_p
        del self.linear_a_g
        del self.linear_b_p
    ...
    def _inference_forward(self,
        ...
        """
        if not hasattr(self, 'linear_a_p'):
            self._init_inference_weights()
    ...
    def forward(self,
        ...
        if faster_alphafold_config.triangle_multiplicative_update:
            if z.dim() == 5:
                change_z_dim = True
                z = z.view(z.nelement() // (N_res * N_res * self.c_z), N_res, N_res, self.c_z)
            abg = self.linear_a_pg_b_pg_g(z)
            if (abg.dim() == 3):
                # abg op assumes 4-dims
                a, b, g = self.triangle_update_abg(torch.unsqueeze(abg, 0), torch.unsqueeze(mask, 0))
                a, b, g = torch.squeeze(a, 0), torch.squeeze(b, 0), torch.squeeze(g, 0)
            else:
                a, b, g = self.triangle_update_abg(abg, mask)
        else:
            a = mask
        ...
        z = x * g
        if change_z_dim:
            z = z.view(z.nelement() // (N_res * N_res * self.c_z), 1, N_res, N_res, self.c_z)
        return z
