...
from opencomplex.data.data_transforms import make_seq_mask, add_distillation_flag, make_all_atom_butype, fix_templates_butype, \
...
class TestDataTransforms(unittest.TestCase):
    def test_make_seq_mask(self):
        ...
        protein_butype = seq_one_hot.clone().detach()
        protein = {'butype': protein_butype}
    ...
    def test_make_all_atom_butype(self):
        seq = torch.tensor([range(20)], dtype=torch.int64).transpose(0, 1)
        seq_one_hot = torch.FloatTensor(seq.shape[0], 20).zero_()
        seq_one_hot.scatter_(1, seq, 1)
        protein_butype = seq_one_hot.clone().detach()
        protein = {'butype': protein_butype}
        protein = make_all_atom_butype(protein)
        assert 'all_atom_butype' in protein
        assert protein['all_atom_butype'].shape == protein['butype'].shape

    def test_fix_templates_butype(self):
        ...
        template_butype = template_seq_one_hot.clone().detach().unsqueeze(0)
        protein = {'template_butype': template_butype}
        protein = fix_templates_butype(protein)
        template_seq_ours = torch.tensor([[0, 4, 3, 6, 13, 7, 8, 9, 11, 10, 12, 2, 14, 5, 1, 15, 16, 19, 17, 18]*2])
        assert torch.all(torch.eq(protein['template_butype'], template_seq_ours))
    ...
    def test_squeeze_features(self):
        ...
        protein = {'butype': torch.tensor(features['butype'])}
    ...
    def test_randomly_replace_msa_with_unknown(self):
        ...
        protein = {'msa': torch.tensor(features['msa']),
                   'butype': torch.argmax(torch.tensor(features['butype']), dim=1)}
        ...
        unknown_proportion_in_seq = torch.bincount(protein['butype'].flatten()) / torch.numel(protein['butype'])
    ...
    def test_make_atom14_masks(self):
        ...
        protein = {'butype': torch.tensor(features['butype'][0])}
