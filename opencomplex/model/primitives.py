...
from opencomplex.utils.checkpointing import get_checkpoint_fn
from opencomplex.utils.chunk_utils import _chunk_slice
from opencomplex.utils.kernel.attention_core import attention_core
...
from opencomplex.faster_alphafold.faster_alphafold import LayerNormFunction, SoftmaxFunction
from opencomplex.faster_alphafold.faster_alphafold import faster_matmul, attention_split_qkv
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        if faster_alphafold_config.layer_norm:
...
def softmax_no_cast(t: torch.Tensor, dim: int = -1) -> torch.Tensor:
    """
        ...
        type bfloat16
    """
    if faster_alphafold_config.softmax and np.prod(t.shape[:-1]) < softmax_limit:
        return SoftmaxFunction.apply(t)
...
#@torch.jit.script
def _attention_v2(query_key_value: torch.Tensor, biases: List[torch.Tensor], scale, head_num) -> torch.Tensor:
    query, key, value = attention_split_qkv(query_key_value, head_num)

    a = faster_matmul(query, key, transpose_b = True, scale = scale)
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.enable_attention_v2 = faster_alphafold_config.attention and self.c_q == self.c_k
        if self.enable_attention_v2:
            linear_q = Linear(
            ...
            self.linear_qkv = Linear(self.c_q, self.c_hidden * self.no_heads * 3, bias=False)
            self.linear_qkv.weight.data.copy_(torch.cat((linear_q.weight.data, linear_k.weight.data, linear_v.weight.data), 0))
        else:
    ...
    def _prep_qkv(self,
        ...
        # [*, Q/K/V, H * C_hidden]
        if self.enable_attention_v2 and (id(q_x) == id(kv_x) or torch.equal(q_x, kv_x)):
            qkv = self.linear_qkv(q_x)
            unit = self.c_hidden * self.no_heads
            q = qkv[..., :unit]
            k = qkv[..., unit:unit*2]
            v = qkv[..., unit*2:]
        else:
    ...
    def forward(
        ...
        if self.enable_attention_v2 and id(q_x) == id(kv_x):
            qkv = self.linear_qkv(q_x)
            o = _attention_v2(qkv, biases, 1.0 / math.sqrt(self.c_hidden), self.no_heads)
